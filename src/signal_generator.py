...
class SignalGenerator:
    ...
    def generate_signal(
        ...
        reasoning = {
            ...
            "group_contributions": {
                "sentiment": round(
                    sentiment_signal_score  * cw["sentiment_signal"] +
                    sentiment_recency_score * cw["sentiment_recency"], 2),
                "technical": round(
                    sma_trend_score      * cw["sma_trend"]      +
                    rsi_momentum_score   * cw["rsi_momentum"]   +
                    macd_signal_score    * cw["macd_signal"]    +
                    bb_position_score    * cw["bb_position"]    +
                    stoch_cross_score    * cw["stoch_cross"]    +
                    volume_confirm_score * cw["volume_confirm"], 2),
                "risk": round(
                    volatility_risk_score * cw["volatility_risk"] +
                    sr_proximity_score    * cw["sr_proximity"]    +
                    trend_strength_score  * cw["trend_strength"]  +
                    rr_quality_score      * cw["rr_quality"],      2),
            },
            "reasoning": {
                "sentiment": {
    ...
    def _calculate_levels(
        ...
        # SHORT (SELL) trade-ek daytrade multipliereket használnak — szűkebb SL/TP,
        # hogy a napi range-en belül befutható legyen a célár (EOD kényszerzárás miatt).
        # LONG (BUY) trade-ek swing multipliereket használnak — szélesebb, multi-day range.
        if "SELL" in decision:
            ...
            # Volatility-adjusted TP multiplier — daytrade SHORT
            if atr_pct < config.take_profit_vol_low_threshold:
                atr_tp_mult = config.short_atr_tp_low_vol       # ~1.0× — intraday TP, calm market
            elif atr_pct > config.take_profit_vol_high_threshold:
                atr_tp_mult = config.short_atr_tp_high_vol      # ~1.8× — intraday TP, volatile
            else:
                t = (atr_pct - config.take_profit_vol_low_threshold) / (
                    config.take_profit_vol_high_threshold - config.take_profit_vol_low_threshold
                )
                atr_tp_mult = config.short_atr_tp_low_vol + t * (
                    config.short_atr_tp_high_vol - config.short_atr_tp_low_vol
                )
            print(f"  📉 SHORT daytrade: ATR%={atr_pct:.2f}% → SL={atr_sl_mult:.2f}×ATR, TP={atr_tp_mult:.2f}×ATR")
        else:
            ...
            # Volatility-adjusted ATR multiplier for TP — swing LONG
        ...
        # Step 1: SL max cap — SHORT daytrade: 1.5%, LONG swing: 5%
        effective_sl_max_pct = config.short_sl_max_pct if "SELL" in decision else config.sl_max_pct
        sl_max_distance = entry_price * effective_sl_max_pct
